          ...
          >
            ...
            <div className="text-[10px] opacity-70">
              {(() => {
                ...
                return `13:00 - ${formatTimeDisplay(endH, endM)} (${Math.floor(style.durationHours)}h total)`;
              })()}
            </div>
            {visibleMetaFields.map((field: any) => {
              const val = formatFieldValueUtil(item, field, collections);
      ...
      {dayDuration > 1 && (
        <div className="text-[10px] opacity-70">
          {(() => {
            ...
            return `${formatTimeDisplay(startH, startM)} - ${formatTimeDisplay(endH, endM)} (${Math.floor(style.durationHours)}h total)`;
          })()}
        </div>
      )}
      {visibleMetaFields.map((field: any) => {
